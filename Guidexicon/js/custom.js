...
for( i=0; i< editor.length; i++ ) {
    editor[i].setAttribute("id" , "editor-" + i);
    ClassicEditor
...
$('.support__carousel.__1').owlCarousel({
    mouseDrag: true,
    touchDrag: true,
    dots: true,
    autoplay: false,
...
$(".__sticky-1").stick_in_parent({
    offset_top: header
});
if ($(window).width() < 576) {
    $(".__sticky").stick_in_parent({
        offset_top: 50
    });
    $(".__sticky-1").stick_in_parent({
        offset_top: header
    ...
    $(".menu-detail-guide").stick_in_parent({
        offset_top: header - 19
    });
}
$(".menu-list-guide").stick_in_parent({
    offset_top: 20
});
// if ($(window).width() < 990) {
//     $(".menu-list-guide").trigger("sticky_kit:detach");
// }
...
if ($(window).width() < 992) {
    $(".author-box").trigger("sticky_kit:detach");
}
...
$(document).ready(function() {
    var a = $('.title-detail-guide').height()
    var b = $('.author-box').height()
    var c = $('.header').height()
    var e = $('.guide-detail-section').height()
    f = e + c;
    d = a + b + c;
    if($(window).width() < 992) {
        $(window).on('scroll', function() {
            if ($(this).scrollTop() > (d + 10)) { // this refers to window
                $('.author-box-mobile').addClass('show')
            }
            else {
                $('.author-box-mobile').removeClass('show')
            }

            if ($(this).scrollTop() > (f + 10)) { // this refers to window
                $('.author-box-mobile').removeClass('show')
            }
        })
    }
...
var menuDetail = $('.box-menu-sticky').height();
var topMenu = $("#top-menu"),
    topMenuHeight = topMenu.outerHeight() + menuDetail + 45,
    // All list items
...
$(document).on('click', '.box-menu-sticky a[href^="#"]', function(e) {
    // target element id
    ...
    var pos = $id.offset().top - header - menuDetail ;
...
$(window).scroll(function(){
   ...
   menuItems
    ...
    var a = menuItems.filter("[href='#"+id+"']").html()
    // console.log(a)
    $('.btn-menu-mobile span').text(a)
})


var menuDetail1 = $('.menu-detail-guide').height();
var topMenu1 = $(".menu-detail-guide"),
    topMenuHeight1 = topMenu1.outerHeight() - 150,
    // All list items
    menuItems1 = topMenu1.find("a.title"),
    // Anchors corresponding to menu items
...
$(document).on('click', '.menu-detail-guide a[href^="#"]', function(e) {
    // target element id
    ...
    // top position relative to the document
    var pos1 = $id1.offset().top - header + 20 ;
...
$(window).scroll(function(){
    // Get container scroll position
   var fromTop1 = $(this).scrollTop() + header;
   ...
   var id1 = cur1 && cur1.length ? cur1[0].id : "";
   // Set/remove active class
   menuItems1.closest('li').removeClass("active").end().filter("[href='#"+id1+"']").closest('li').addClass("active");
     var a1 = menuItems1.filter("[href='#"+id1+"']").html()
     console.log(a1)
     $('.btn-menu-mobile span').text(a1)
     if (a1 == null) {
        $('.btn-menu-mobile span').text('Menu')
     }
})

$('.btn-menu-mobile').on('click', function() {
    $('.box-menu-sticky ul').toggleClass('show')
    $('.chapter-box').toggleClass('show')
    $('.menu-content').toggleClass('show')
})

$('.box-menu-sticky ul li a').on('click', function() {
    $('.box-menu-sticky ul').removeClass('show')
})
var chapterdrag = $('.chapterdrag')
for (var i = 0; i < chapterdrag.length; i++) {
	new Sortable(chapterdrag[i], {
		animation: 150,
		fallbackOnBody: true,
		swapThreshold: 0.65,
	});
}
var itemchapter = $('.itemchapter')

for (var i = 0; i < itemchapter.length; i++) {
	new Sortable(itemchapter[i], {
		group: 'nested',
		animation: 150,
		fallbackOnBody: true,
		swapThreshold: 0.65,
...
// for (var i = 0; i < contentSort.length; i++) {
// 	new Sortable(contentSort[i], {
// 		// group: 'nested',
// 		animation: 150,
// 		fallbackOnBody: true,
// 		swapThreshold: 0.65,
...
// if ($(window).width() > 991 && $(window).width() < 1200) {
//     $(".menu-detail-guide").stick_in_parent({
//         offset_top: 90 + header 
//     });
// }

// if ($(window).width() > 1199) {
//     $(".menu-detail-guide").stick_in_parent({
//         offset_top: 10 + header 
//     });
// }
$('#check-partner').each(function() {
    $('input', this).on('change', function() {
        if ($(this).is(':checked')) {
            $('.form-partner').fadeIn();
        }
        else {
            $('.form-partner').fadeOut();
        } 
...
// $('.item-chapter').on('mouseenter', function() {
//     $('.add-btn', this).slideDown();
// })
// $('.item-chapter').on('mouseleave', function() {
//     $('.add-btn', this).slideUp();
// })
var upload = 2
var $this = $('.item-container');
$('#add-item').on('click', function(e) {
    e.preventDefault();
    var recRow = '<div class="item-box" id="item-'+upload+'"><div class="form__group"><input type="text" class="form__input" placeholder="Items name"></div><div class="form__gallery"><label id="gallery-'+upload+'" for="form__gallery-upload-'+upload+'">Upload photos</label><input data-name="#gallery1" id="form__gallery-upload-'+upload+'" name="gallery-'+upload+'" class="form__gallery-upload" type="file" accept=".png, .jpg, .jpeg" multiple=""></div><div class="form__group"><div class="editor" id="editor-'+upload+'"></div></div><div class="remove-item"><a href="javascript:;" class="btn"><i class="far fa-trash-alt"></i>Remove</a></div></div>';
    $($this).append(recRow);

      $('#editor-'+upload).each(function(){
        // var $this = $(this)
        ClassicEditor
        .create( document.querySelector('#editor-' + upload) )
        .catch( error => {
            console.error( error );
        } );
      });
...
$('#add-link').on('click', function(e) {
    e.preventDefault();
    var recRow = '<div class="item-box" id="item-'+upload+'"><div class="sign__group select-group"><label class="sign__label" for="link-'+upload+'">Title</label><input type="text" min="0" class="form__input" placeholder=""></div><div class="sign__group select-group"><label class="sign__label" for="link-'+upload+'">Url</label><input type="text" class="form__input" placeholder=""></div><div class="remove-item"><button class="btn remove-btn" type="button">Remove</button></div></div>';
    $($this).append(recRow);
        ...
        $('#item-'+upload).each(function() {
        var $this = $(this)
        $('.remove-item .remove-btn',this).on('click',function() {
            $this.remove()
        })
    })
